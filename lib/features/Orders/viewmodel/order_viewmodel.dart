...
//     notifyListeners();
//     try {
//       // Professional Fix: Fetch all order types concurrently and handle individual API failures.
//       // This ensures that if one endpoint (e.g., completed orders) fails, the app still loads data from others (e.g., assigned).
//       final results = await Future.wait([
//         _repository.getPickupAssignedOrders().catchError((e) {
//           debugPrint("Failed to fetch pickup assigned orders: $e");
//           return <OrderModel>[]; // Return empty list on error
//         }),
//         _repository.getDeliveryAssignedOrders().catchError((e) {
//           debugPrint("Failed to fetch delivery assigned orders: $e");
//           return <OrderModel>[];
//         }),
//         _repository.getAllOrders().catchError((e) {
//           debugPrint("Failed to fetch completed orders: $e");
//           return <OrderModel>[];
//         }),
//       ]);

//       final fetchedOrders = results.expand((orders) => orders).toList();
...
//       for (final newOrder in fetchedOrders) {
//         final stage = existingStages["${newOrder.orderId}_${newOrder.orderType}"];
//         uniqueOrders["${newOrder.orderId}_${newOrder.orderType}"] = stage != null ? newOrder.copyWith(deliveryStage: stage) : newOrder;
//       }
...
//   Future<bool> sendDeliveryOtp(BuildContext context, String orderId) async {
//     _errorMessage = null;
//     notifyListeners();
//     try {
//       final Map<String, dynamic> result = await _repository.sendDeliveryOtp(orderId);
...
//         return true;
//       } else {
//         throw Exception(result['msg'] ?? "API failed to send OTP");
//       }
//     } catch (e) {
...
//   Future<bool> verifyDeliveryOtp(BuildContext context, String orderId, String otp) async {
//     _errorMessage = null;
//     notifyListeners();
//     try {
//       final Map<String, dynamic> result = await _repository.verifyDeliveryOtp(orderId, otp);
//       if (result['success'] == true) {
//         return true;
//       }
//       final message = result['msg']?.toString() ?? "Invalid OTP. Please try again.";
//       AppToast.showError(context: context, title: "Verification Failed", message: message);
//       throw Exception(message);
//     } catch (e) {
//       debugPrint("verifyDeliveryOtp Error: $e");
//       _errorMessage = "Failed to verify OTP: $e";
//       AppToast.showError(context: context, title: "Verification Failed", message: _errorMessage!);
//       notifyListeners();
//       return false;
//     }
//   }
...
// }
  ...
  class OrderViewModel extends ChangeNotifier {
    ...
    // Logic Getters for UI
    List<OrderModel> get allFilteredOrders =>
        _orders.where((o) => o.orderType == _selectedOrderType).toList();
    List<OrderModel> get pendingOrders => _orders
        .where((o) =>
            o.orderType == _selectedOrderType && o.status == OrderStatus.pending)
        .toList();
    List<OrderModel> get assignedOrders => _orders
        .where((o) =>
            o.orderType == _selectedOrderType && o.status == OrderStatus.assigned)
        .toList();
    List<OrderModel> get completedOrders => _orders
        .where((o) =>
            o.orderType == _selectedOrderType && o.status == OrderStatus.completed)
        .toList();
    void clearAllCachedData() {
      _orders.clear();
      _isLoading = false;
    ...
    Future<void> fetchAllOrders() async {
      ...
      notifyListeners();
      try {
        // Professional Fix: Fetch all order types concurrently from their specific endpoints.
        // This ensures data is correctly segregated and robust against individual API failures.
        final results = await Future.wait<List<OrderModel>>([
          // Fetch new, unassigned orders
          _repository.getPickupOrders().catchError((e) {
            debugPrint("Failed to fetch pending pickup orders: $e");
            return <OrderModel>[];
          }),
          _repository.getDeliveryOrders().catchError((e) {
            debugPrint("Failed to fetch pending delivery orders: $e");
            return <OrderModel>[];
          }),
          _repository.getPickupAssignedOrders().catchError((e) {
            debugPrint("Failed to fetch pickup assigned orders: $e");
          ...
          _repository.getPickupCompletedOrders().catchError((e) {
            debugPrint("Failed to fetch pickup completed orders: $e");
            return <OrderModel>[];
          }),
          _repository.getDeliveryCompletedOrders().catchError((e) {
            debugPrint("Failed to fetch delivery completed orders: $e");
            return <OrderModel>[];
          }),
        ]);

        final List<OrderModel> fetchedOrders = results.expand((orders) => orders).toList();
        ...
        _orders = fetchedOrders.map<OrderModel>((newOrder) {
          final stage = existingStages["${newOrder.orderId}_${newOrder.orderType}"];
    ...
    /// Sets the currently selected order type to filter the lists.
    void setSelectedOrderType(OrderType type) {
    ...
    Future<void> verifyOrder(String orderId) async {
      ...
      try {
        ...
        await _repository.verifyOrder(orderId);
        // After successful verification, refresh to get the updated order state
        await fetchAllOrders();
      } catch (e) {
        debugPrint("CONSOLE: Verify Error: $e");
    ...
    Future<Map<String, dynamic>> sendDeliveryOtp(BuildContext context, String orderId) async {
      _errorMessage = null;
      notifyListeners();
      try {
        final result = await _repository.sendDeliveryOtp(orderId);
        if (result['success'] == true) {
          return result;
        } else {
          throw Exception(result['msg'] ?? "API failed to send OTP");
    ...
    Future<bool> verifyDeliveryOtp(BuildContext context, String orderId, String otp) async {
      ...
      notifyListeners();
      try {
        final result = await _repository.verifyDeliveryOtp(orderId, otp);
        if (result['success'] == true) {
          return true;
        }
        final message = result['msg']?.toString() ?? "Invalid OTP. Please try again.";
