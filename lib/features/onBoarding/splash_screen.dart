...
class _SplashScreenState extends State<SplashScreen>
  ...
  void initState() {
    ...
    _iconSlide = Tween<Offset>(begin: const Offset(0, -0.8), end: Offset.zero)
        .animate(
          CurvedAnimation(
            parent: _mainController,
    ...
    _textSlide = Tween<Offset>(begin: const Offset(1.2, 0), end: Offset.zero)
        .animate(
          CurvedAnimation(
            parent: _mainController,
  ...
  Future<void> _navigateToNext() async {
    ...
    final onboardingCompleted = await storage.read(key: 'onboardingCompleted');

    if (onboardingCompleted != 'true') {
      // New user flow: Navigate to Onboarding screen
      if (mounted) {
    ...
    if (token != null && token.isNotEmpty) {
      try {
        ...
        /// INIT NOTIFICATIONS
        final profile = context.read<ProfileViewModel>().userProfile;
        ...
        if (notificationService.pendingNavigationData != null) {
          final data = notificationService.pendingNavigationData!;
          notificationService.pendingNavigationData = null;
          ...
          WidgetsBinding.instance.addPostFrameCallback((_) {
            notificationService.handlePendingNavigation(data, context);
          });
